...
// OP_MOVE Tests: R[A] := R[B], if C != 0, pc += (kc ? C : -C)
TEST_F(VMInstructionGeneralTest, OpMoveBasicRegisterCopy) {
    Instruction code[2];
    code[0] = INSTRUCTION_MOVE(1, 0, 0, false, false);  // C=0, no jump
    code[1] = INSTRUCTION_TRAP(0, 0, false, false);
    ...
    ASSERT_EQ(vm->values[1].as.i, 123) << "Register 1 should contain value 123";
}

TEST_F(VMInstructionGeneralTest, OpMovePositiveJumpWithConstant) {
    Instruction code[4];
    code[0] = INSTRUCTION_MOVE(1, 0, 2, false, true);  // C=2, kc=true, pc += 2
    code[1] = INSTRUCTION_TRAP(0, 99, false, false);
    code[2] = INSTRUCTION_TRAP(0, 0, false, false);
    code[3] = INSTRUCTION_TRAP(0, 88, false, false);
    ...
    ASSERT_EQ(result, 0) << "VM should complete successfully";
    ASSERT_EQ(vm->values[1].header, VALUE_TYPE_INT) << "Register 1 should have int type";
    ASSERT_EQ(vm->values[1].as.i, 456) << "Register 1 should contain value 456";
}

TEST_F(VMInstructionGeneralTest, OpMoveNegativeJumpWithConstant) {
    Instruction code[4];
    code[0] = INSTRUCTION_JUMP(2, true);
    code[1] = INSTRUCTION_TRAP(0, 0, false, false);
    code[2] = INSTRUCTION_MOVE(2, 0, 1, false, false);
    code[3] = INSTRUCTION_TRAP(0, 99, false, false);
    ...
    ASSERT_EQ(vm->values[2].as.i, 321) << "Register 2 should contain value 321";
}

TEST_F(VMInstructionGeneralTest, OpMovePositiveJumpWithRegister) {
    Instruction code[4];
    code[0] = INSTRUCTION_MOVE(1, 0, 2, true, true);
    code[1] = INSTRUCTION_TRAP(0, 99, false, false);
    code[2] = INSTRUCTION_TRAP(0, 0, false, false);
    code[3] = INSTRUCTION_TRAP(0, 88, false, false);
    ...
    ASSERT_EQ(result, 0) << "VM should complete successfully";
    ASSERT_EQ(vm->values[1].header, VALUE_TYPE_INT) << "Register 1 should have int type";
    ASSERT_EQ(vm->values[1].as.i, 789) << "Register 1 should contain value 789";
}

TEST_F(VMInstructionGeneralTest, OpMoveNegativeJumpWithRegister) {
    Instruction code[4];
    code[0] = INSTRUCTION_JUMP(2, true);
    code[1] = INSTRUCTION_TRAP(0, 0, false, false);
    code[2] = INSTRUCTION_MOVE(3, 0, 1, true, false);
    code[3] = INSTRUCTION_TRAP(0, 88, false, false);
    ...
    ASSERT_EQ(vm->values[3].as.i, 555) << "Register 3 should contain value 555";
}

TEST_F(VMInstructionGeneralTest, OpMoveLargerJump) {
    Instruction code[7];
    code[0] = INSTRUCTION_MOVE(1, 0, 5, false, true);  // C=5, kc=true, pc += 5
    code[1] = INSTRUCTION_TRAP(0, 99, false, false);
    code[2] = INSTRUCTION_TRAP(0, 99, false, false);
    code[3] = INSTRUCTION_TRAP(0, 99, false, false);
    code[4] = INSTRUCTION_TRAP(0, 99, false, false);
    code[5] = INSTRUCTION_TRAP(0, 0, false, false);
    code[6] = INSTRUCTION_TRAP(0, 88, false, false);

    vm->values[0].header = VALUE_TYPE_INT;
    vm->values[0].as.i   = 999;

    SemiModule* module  = semiVMModuleCreate(&vm->gc, SEMI_REPL_MODULE_ID);
    FunctionProto* func = CreateFunctionObject(0, code, 7, 8, 0, 0);
    module->moduleInit  = func;
    ...
    ASSERT_EQ(result, 0) << "VM should complete successfully";
    ASSERT_EQ(vm->values[1].header, VALUE_TYPE_INT) << "Register 1 should have int type";
    ASSERT_EQ(vm->values[1].as.i, 999) << "Register 1 should contain value 999";
}

TEST_F(VMInstructionGeneralTest, OpMoveSequentialWithoutJumps) {
    Instruction code[4];
    code[0] = INSTRUCTION_MOVE(2, 1, 0, false, false);  // C=0, no jump
    code[1] = INSTRUCTION_MOVE(0, 2, 0, false, false);  // C=0, no jump
    code[2] = INSTRUCTION_MOVE(3, 0, 0, false, false);  // C=0, no jump
